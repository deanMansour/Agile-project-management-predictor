...
class home_page_view(View):
    ...
    def post(self, request):
        ## there is no button in homepage.html that submit to this class url path
        # data_to_render = {'display': "Home Page"}
        # return render(request, 'Predicting_app/home_page.html',{'data':data_to_render})
        return None
#--------------------------------------------------------------------------------
def logout_page_view(self, request):
...
class loging_page_view(View):
    def get(self, request):
        ...
        return render(request, 'Predicting_app/homepage.html', {'form': form, 'data':data_to_render })
    #----------------------
    def post(self, request):
        if request.user.is_authenticated:
...
class signup_page_view(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            #form.save()-->save UserCreation_Form class instance and return the instance of its linked model class
            user_object = form.save()
            user_object.refresh_from_db()
            # save the new created model class instance that linked to UserCreation_Form class
            user_object.save()
            return redirect('home_page_view_path')
        else:
            form = UserCreation_Form()
